...
The backend no longer stores rules of type `CONTAINER`, so there is nothing left to import or
refresh. Replace the `sysdig_secure_rule_container` block with a
